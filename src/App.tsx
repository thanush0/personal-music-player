...
// Local Player
import LocalPlayerProvider, { LocalPlayerProviderProps } from './utils/localPlayer/LocalPlayerProvider';
...
const SpotifyContainer: FC<{ children: any }> = memo(({ children }) => {
  ...
  useEffect(() => {
    // For local player, we don't need Spotify authentication
    // Just fetch the mock user
    dispatch(authActions.fetchUser());
  }, [dispatch]);

  const localPlayerProps: LocalPlayerProviderProps = useMemo(
    () => ({
      onPlayerLoading: () => {},
      onPlayerError: (e) => {
  ...
  return <LocalPlayerProvider {...localPlayerProps}>{children}</LocalPlayerProvider>;
});
  ...
  const routes = useMemo(
    () =>
      [
        ...
        { path: '/download', element: <DownloadPage />, public: true },
