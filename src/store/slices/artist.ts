...
export const fetchArtist = createAsyncThunk<[Artist, boolean, TrackWithSave[], Album[][]], string>(
  ...
  async (id, params) => {
    ...
    const promises = [
      ...
      // Remove legacy top-tracks endpoint - not supported by local backend
    ...
    // Only check saved tracks if there are any tracks
    const extraResponses = tracks.length > 0 
      ? await Promise.all([
...
const fetchOtherArtists = createAsyncThunk<Artist[], string>(
  ...
  async (id) => {
    // Legacy endpoint - not supported by local backend
